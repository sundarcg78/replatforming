...
import org.superbiz.moviefun.common.Blob;
import org.superbiz.moviefun.common.BlobStore;
import org.superbiz.moviefun.common.FileStore;
...
public class AlbumsController {
    ...
    public HttpEntity<byte[]> getCover(@PathVariable long albumId) throws IOException {
        ...
        Blob blob = optionalBlob.orElseGet(this::getDefaultCoverBlob);
        byte[] coverImageArray = IOUtils.toByteArray(blob.inputStream);
        HttpHeaders headers = createImageHttpHeaders(blob, coverImageArray);

        return new HttpEntity<>(coverImageArray, headers);
    }
    ...
    private String getCoverFile(@PathVariable long albumId) {
        System.out.println("CoverFileName::" + format("covers/%d", albumId));
